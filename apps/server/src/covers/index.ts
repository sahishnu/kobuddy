...
export async function listCoverCandidates(
  ...
  return searchCoverCandidates(title, authors, isbn, cfg.GOOGLE_BOOKS_API_KEY);
}

export async function listIsbnCandidates(
  cfg: AppConfig,
  title: string,
  authors: string,
): Promise<IsbnCandidate[]> {
  return searchIsbnCandidates(title, authors, cfg.GOOGLE_BOOKS_API_KEY);
}
