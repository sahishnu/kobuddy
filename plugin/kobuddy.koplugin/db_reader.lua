...
function KoInsightDbReader.getCurrentDocumentPages(book_md5)
  ...
  if rows > 0 and result[1] and result[1][1] then
    ...
    if current_md5 == book_md5 then
      ...
      logger.info(
        string.format(
          "[kobuddy] Using live page count for book %s: %d",
          doc_props.title,
          page_count
        )
      )
...
function KoInsightDbReader.bookData()
  ...
  for i = 1, rows do
    ...
    if current_pages and current_pages ~= db_pages then
      logger.info(
        string.format(
          "[kobuddy] Using live page count for book %s: %d (DB has: %d)",
          result[2][i],
          current_pages,
          db_pages
        )
...
local function flush_statistics_to_db()
  ...
  if ui and ui.statistics and ui.statistics.is_doc then
    ...
    if ok_flush then
      logger.info("[kobuddy] Flushed statistics to DB before sync")
    else
      logger.warn("[kobuddy] Failed to flush statistics to DB: " .. tostring(err))
    end
  end
end
...
function KoInsightDbReader.progressData()
  ...
  for i = 1, rows do
    ...
    if book_md5 == nil then
      logger.warn("[kobuddy] Book MD5 not found in book data:" .. book_id)
      goto continue
    end
